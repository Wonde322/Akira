...
Результат всегда структурированный: код возврата, stdout, stderr,
лимит вывода и флаги обрезания. При таймауте процесс убивается
...
def _run(command, timeout, working_dir):
    """Запускает команду и возвращает результат выполнения или таймаут."""
    ...
    try:
        ...
        return {
            ...
            "exit_code": None,
            "stdout": "",
            "stderr": "Команда убита по таймауту.",
        }
...
def shell(command, timeout=None, cwd=None):
    """Выполняет команду в оболочке с таймаутом и лимитом вывода."""
    ...
    if (
        ...
        or not 1 <= timeout <= MAX_SHELL_TIMEOUT
    ):
        return fail(
            "invalid_timeout",
            "timeout должен быть числом от 1 до " + str(MAX_SHELL_TIMEOUT) + ".",
        )
    ...
    if cwd is not None:
        try:
            working_dir = str(
                resolve_path(cwd, require_existing=True, must_be_dir=True)
            )
