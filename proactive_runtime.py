...
class ProactiveRuntime:
    ...
    def _message_for_failure(payload):
        goal = str(payload.get("goal") or "фоновой задачи").strip()
    ...
    def decide(self, event, trigger_goals=None):
        ...
        with self._lock:
            ...
            if last is not None and now - last < self.dedupe_seconds:
                return ProactiveDecision(ProactiveAction.IGNORE, "duplicate_event")
    ...
    def _push_attention(self, event, decision):
        if decision.action not in {ProactiveAction.NOTIFY, ProactiveAction.ASK_USER}:
            return None
        inbox = self._inbox
        if inbox is None:
            from proactive_inbox import get_proactive_inbox
            inbox = get_proactive_inbox()
        return inbox.push(decision.notification, action=decision.action.value, event=event,
    ...
    def handle(self, event, trigger_goals=None):
        ...
        if spawn_result.get("success"):
            result["launched"].append({"task_id": spawn_result.get("task_id"), "reason": decision.reason})
        return result
