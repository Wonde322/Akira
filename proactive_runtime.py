...
This layer sits between raw events and autonomous work. It keeps the
EventBus transport-focused and decides whether an event should be ignored,
recorded, or turned into a background task.
"""
...
class ProactiveRuntime:
    """Policy and safety layer for event-driven proactive behavior."""

    def __init__(
        self,
        dedupe_seconds=5.0,
        desktop_cooldown_seconds=30.0,
        max_causation_depth=MAX_CAUSATION_DEPTH,
        clock=None,
    ):
        self.dedupe_seconds = float(dedupe_seconds)
        self.desktop_cooldown_seconds = float(
            desktop_cooldown_seconds
        )
    ...
    def _depth(self, event):
        try:
            return int(event.get("causation_depth") or 0)
        except Exception:
            return 0
    ...
    def decide(self, event, trigger_goals=None):
        ...
        with self._lock:
            ...
            cutoff = now - max(self.dedupe_seconds * 4, 60.0)
            self._recent = {
                key: value
            ...
            if (
                event_type == "desktop.changed"
                and last_type is not None
                and now - last_type < self.desktop_cooldown_seconds
            ):
                return ProactiveDecision(
        ...
        if event_type == "schedule.due":
            goal = str(
                (event.get("payload") or {}).get("goal") or ""
            ).strip()
            if not goal:
                return ProactiveDecision(
                    ProactiveAction.IGNORE,
                    "scheduled_event_without_goal",
                )
            return ProactiveDecision(
                ProactiveAction.SPAWN_TASK,
                "explicit_schedule",
                goal=goal,
                source="scheduler",
            )
        ...
        if event_type == "task.failed":
            return ProactiveDecision(
                ...
                notification="Фоновая задача Акиры завершилась ошибкой.",
            )

        return ProactiveDecision(
            ProactiveAction.RECORD,
            "no_actionable_policy",
        )
    ...
    def handle(self, event, trigger_goals=None):
        ...
        if decision.action != ProactiveAction.SPAWN_TASK:
            return result
        ...
        spawn_result = get_runtime().spawn(
            ...
            causation_depth=self._depth(event) + 1,
