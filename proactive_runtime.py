...
class ProactiveRuntime:
    ...
    def _record(self, event, decision):
        ...
        with self._lock:
            self._decisions.append(item); del self._decisions[:-100]
        return item
    ...
    def decide(self, event, trigger_goals=None):
        ...
        with self._lock:
            ...
            cutoff = now - max(self.dedupe_seconds * 4, 60.0); self._recent = {k:v for k,v in self._recent.items() if v >= cutoff}
