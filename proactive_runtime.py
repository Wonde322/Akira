...
class ProactiveRuntime:
    def __init__(self, dedupe_seconds=5.0, desktop_cooldown_seconds=30.0,
        ...
        rules = list(context_rules) if context_rules is not None else self._context_rule_store.active()
        self._context_triggers = ContextTriggerEngine(rules)
    ...
    def _record(self, event, decision):
        ...
        return item
    @staticmethod
    def _message_for_failure(payload):
    ...
    def _context_decision(self, payload):
        ...
        match = matches[0]; rule = match["rule"]
        ...
        return ProactiveDecision(action, "context_rule:" + rule["id"], notification=match["message"], source="context_trigger", priority=priority)
    ...
    def decide(self, event, trigger_goals=None):
        ...
        if event_type == "schedule.due":
            ...
            return ProactiveDecision(ProactiveAction.SPAWN_TASK, "explicit_schedule", goal=goal, source="scheduler", priority="high") if goal else ProactiveDecision(ProactiveAction.IGNORE, "scheduled_event_without_goal")
    ...
    def handle(self, event, trigger_goals=None):
        event_type = str(event.get("type") or "")
        payload = event.get("payload") or {}
        lifecycle = self._update_lifecycle(event_type, payload)
        ...
        spawn_result = self._spawn_task(get_runtime(), decision.goal, "proactive:" + str(correlation_id), parent_event_id=parent_event_id, correlation_id=correlation_id, causation_depth=self._depth(event))
