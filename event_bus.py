...
def _runtime_mapping(value):
    return value if isinstance(value, dict) else {}

def _runtime_launched(value):
...
class EventBus:
    ...
    def _load_triggers(self):
        ...
        if isinstance(data, list):
            for trigger in data[-MAX_TRIGGERS:]:
                if not isinstance(trigger, dict): continue
                raw_id = trigger.get("id")
                if isinstance(raw_id, bool) or not isinstance(raw_id, (str, int)): continue
                trigger_id = str(raw_id).strip(); event_type = str(trigger.get("event_type") or "").strip(); goal = str(trigger.get("goal") or "").strip()
                if not trigger_id or not event_type or not goal: continue
                trigger["id"] = trigger_id; trigger["event_type"] = event_type; trigger["goal"] = goal
    ...
    def create_trigger(self, event_type, goal, cooldown_seconds=0):
        ...
        trigger = {"id": tid, "event_type": event_type, "goal": goal, "enabled": True, "cooldown_seconds": cooldown, "created_at": _iso(), "last_fired_at": None, "fire_count": 0, "last_task_id": None, "last_error": None, "recent_correlations": []}
    ...
    def _remember_correlation(self, trigger, correlation_id):
        if correlation_id:
            values = [str(value) for value in trigger.get("recent_correlations") or [] if str(value) != str(correlation_id)]; values.append(str(correlation_id)); trigger["recent_correlations"] = values[-MAX_TRIGGER_CORRELATIONS:]
    def _forget_correlation(self, trigger, correlation_id):
        if correlation_id: trigger["recent_correlations"] = [str(value) for value in trigger.get("recent_correlations") or [] if str(value) != str(correlation_id)]
    ...
    def _record_results(self, results, correlation_id):
        with self._lock:
            for trigger, result in results:
                ...
                if isinstance(result, Exception): current["last_error"] = str(result); self._forget_correlation(current, correlation_id); continue
                spawn = _runtime_mapping(result.get("spawn")); decision = _runtime_mapping(result.get("decision"))
                if spawn.get("success"):
                    current["last_fired_at"] = _iso(); current["fire_count"] = max(0, _persisted_int(current.get("fire_count"), 0)) + 1; current["last_task_id"] = spawn.get("task_id"); current["last_error"] = None
                elif decision.get("action") == "spawn_task": current["last_error"] = spawn.get("error") or spawn.get("output") or "spawn_failed"; self._forget_correlation(current, correlation_id)
    ...
    def emit(self, event_type, payload=None, *, parent_event_id=None, correlation_id=None, causation_depth=0, source="system"):
        eid = uuid.uuid4().hex[:16]; event_type = _metadata_text(event_type, "")
        if not event_type: return {"success": False, "error": "empty_event_type", "output": "Не указан event_type."}
        safe_payload = _json_safe(payload, {}) if isinstance(payload, dict) else {}; safe_payload = safe_payload if isinstance(safe_payload, dict) else {}
        parent_event_id = _metadata_text(parent_event_id); correlation_id = _metadata_text(correlation_id) or parent_event_id or eid; source = _metadata_text(source, "system")
        ...
        if eligible:
            for trigger in eligible:
                ...
                try:
                    ...
                    if not isinstance(result, dict): raise TypeError("proactive_runtime returned non-dict result")
        ...
        launched = [item for _, result in results if not isinstance(result, Exception) for item in _runtime_launched(result.get("launched"))]
    ...
    def _cooldown_active(self, trigger):
        cooldown = _persisted_int(trigger.get("cooldown_seconds"), 0); last = trigger.get("last_fired_at")
        ...
        elapsed = (now - timestamp).total_seconds(); return 0 <= elapsed < cooldown

    def _render_goal(self, goal, event):
        payload = event.get("payload", {}); result = str(goal)
        for key, value in {"{{event.type}}": str(event.get("type", "")), "{{event.id}}": str(event.get("id", "")), "{{event.timestamp}}": str(event.get("timestamp", "")), "{{event.payload}}": json.dumps(payload, ensure_ascii=False, default=str)}.items(): result = result.replace(key, value)
        return result
