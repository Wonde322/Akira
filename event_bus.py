...
class EventBus:
    ...
    def _save_triggers(self):
        ...
        tmp.write_text(json.dumps(list(self._triggers.values())[-MAX_TRIGGERS:], ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    def create_trigger(self, event_type, goal, cooldown_seconds=0):
        ...
        if not goal:
            return {"success": False, "error": "empty_goal", "output": "Не указана trigger goal."}
        try:
            cooldown = max(0, int(cooldown_seconds or 0))
        except Exception:
            return {"success": False, "error": "invalid_cooldown", "output": "cooldown_seconds должен быть числом."}
    ...
    def cancel_trigger(self, trigger_id):
        with self._lock:
            ...
            if trigger is None:
                return {"success": False, "error": "trigger_not_found", "output": f"Trigger {trigger_id} не найден."}
    ...
    def list_triggers(self, limit=100):
        ...
        with self._lock:
            data = list(self._triggers.values())[-limit:]
            data.reverse()
    ...
    def _correlation_seen(self, trigger, correlation_id):
        return bool(correlation_id) and str(correlation_id) in set(str(value) for value in trigger.get("recent_correlations") or [])
    ...
    def _remember_correlation(self, trigger, correlation_id):
        ...
        correlations.append(str(correlation_id))
        trigger["recent_correlations"] = correlations[-MAX_TRIGGER_CORRELATIONS:]
    ...
    def _trigger_still_enabled(self, trigger_id, correlation_id):
        with self._lock:
            ...
            if current is not None and current.get("enabled"):
                return True
            if current is not None:
                self._forget_correlation(current, correlation_id)
                self._save_triggers()
        return False
        ...
        with self._lock:
            for trigger, result in results:
                ...
                if spawn.get("success"):
                    ...
                    current["last_error"] = spawn.get("error") or spawn.get("output")
                    self._forget_correlation(current, correlation_id)
                else:
                    self._forget_correlation(current, correlation_id)
            self._save_triggers()
    ...
    def emit(self, event_type, payload=None, *, parent_event_id=None, correlation_id=None, causation_depth=0, source="system"):
        ...
        with self._lock:
            ...
            for trigger in eligible:
                current = self._triggers.get(trigger["id"])
                if current is not None:
                    self._remember_correlation(current, event.get("correlation_id"))
            if eligible:
                self._save_triggers()
        try:
            ...
            with self._lock:
                for trigger in eligible:
                    current = self._triggers.get(trigger["id"])
                    if current is not None:
                        self._forget_correlation(current, event.get("correlation_id"))
                if eligible:
                    self._save_triggers()
        ...
        if eligible:
            for trigger in eligible:
                if not self._trigger_still_enabled(trigger["id"], event.get("correlation_id")):
                    continue
                dispatched = dict(event)
...
def create_trigger(event_type, goal, cooldown_seconds=0):
    return get_event_bus().create_trigger(event_type, goal, cooldown_seconds)


def cancel_trigger(trigger_id):
    return get_event_bus().cancel_trigger(trigger_id)


def list_triggers(limit=100):
    return get_event_bus().list_triggers(limit)


def emit_event(event_type, payload=None, **metadata):
    return get_event_bus().emit(event_type, payload, **metadata)
