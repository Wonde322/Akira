...
class ProactiveActionLifecycle:
    ...
    def started(self, task_id, goal, *, kind=None, correlation_id=None):
        item = {"task_id": str(task_id), "goal": str(goal or "").strip(), "kind": kind, "correlation_id": correlation_id, "status": "running", "result": None, "error": None, "started_at": self._clock(), "finished_at": None}
        with self._lock:
        ...
        with self._lock:
            for task_id, item in self._items.items():
                ...
                if status == "completed":
                    ...
                    item["status"] = "failed"; item["error"] = task.get("error"); item["finished_at"] = task.get("finished_at") or self._clock(); changed.append(dict(item))
