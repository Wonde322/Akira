...
from config import COMPUTER_USE_MAX_STEPS, MAX_TOOL_ITERATIONS, NO_PROGRESS_LIMIT, MODEL
...
def execute_tool_result(function_name, arguments, source=None):
    ...
    try:
        ...
        return {"success": False, "error": "execution_error", "output": f"Ошибка выполнения инструмента: {exc}"}
...
def _sync_compat_audit(agent_loop):
    """Bridge reloaded test/runtime audit modules without duplicating audit logic."""
    try:
        import audit
