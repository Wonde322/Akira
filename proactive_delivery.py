...
The runtime decides what deserves attention. This module delivers it and
routes explicit answers or proposal selections back into the event pipeline.
"""
...
class ProactiveDelivery:
    def __init__(self, inbox=None, emit=None, action_executor=None):
    ...
    def _take_pending(self, item_id):
        with self._lock:
            return self._pending_questions.pop(str(item_id), None)
    ...
    def answer(self, item_id, text):
        ...
        event = self._emit(
            "proactive.answer",
            {"answer": text, "question": item.get("message"), "inbox_item_id": item.get("id"), "question_event_id": item.get("event_id")},
        ...
        return {"success": bool(acknowledgement.get("success")), "event": event, "item": acknowledgement.get("item")}

    def select(self, item_id, proposal_id):
        """Execute one explicitly selected proposal for a pending question."""
        key = str(item_id)
        with self._lock:
            item = self._pending_questions.get(key)
        if item is None:
            return {"success": False, "error": "question_not_pending"}
        ...
        acknowledgement = self._inbox.acknowledge(item.get("id"))
        result["item"] = acknowledgement.get("item")
        return result
