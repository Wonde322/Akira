...
def _clear_modules():
    for name in (
        "config", "agent_loop", "analysis", "brain", "permissions", "memory",
        "tool_registry", "session", "audit", "format",
    ):
        sys.modules.pop(name, None)


def test_agent_loop_and_analysis_import_without_groq_api_key(monkeypatch, tmp_path):
    ...
    try:
        assert agent_loop.client is None
...
def test_lazy_client_is_shared_across_canonical_modules(monkeypatch, tmp_path):
    monkeypatch.setenv("GROQ_API_KEY", "test-key")
    monkeypatch.chdir(tmp_path)
    _clear_modules()
    ...
    try:
        assert agent_loop._ensure_client() is config._client
        assert analysis._ensure_client() is config._client
        assert agent_loop.client is analysis.client
    finally:
        _clear_modules()
