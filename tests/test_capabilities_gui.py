...
def test_prompt_on_stdin_is_strict_exact_match(isolated_project, monkeypatch):
    ...
    for polluted in ("2+3=да", "мусорда", "нет", "да\n мусор"):
        monkeypatch.setattr(builtins, "input", lambda _, value=polluted: value)
...
def test_gui_type_does_not_pollute_followup_confirmation(isolated_project, monkeypatch):
    import io
    import sys
    ...
    monkeypatch.setattr(builtins, "input", lambda prompt: stdin_buffer.readline().rstrip("\n"))
    result = gui.type_text("2+3=", target="Calculator")
    assert result["success"] is True
    assert fake.events == [("activate", "Calculator"), ("type", "2+3=")]
...
def test_gui_action_blocked_by_permission_system(isolated_project, monkeypatch):
    ...
    monkeypatch.setattr(agent_loop, "request_confirmation", lambda *args, **kwargs: False)
