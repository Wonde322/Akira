...
class VoiceEngine(QObject):
    ...
    def __init__(self,wake_enabled=True,parent=None):
        super().__init__(parent); self._wake_enabled=wake_enabled; self._dialogue=False; self._commands=queue.Queue(); self._stop_event=threading.Event(); self._interrupt=threading.Event(); self._cancel_event=threading.Event(); self._thread=None; self._audio_ok=False; self._listening=True; self._stop=False
    def start(self):
        if self._thread and self._thread.is_alive(): return
        self._stop=False; self._listening=True; self._stop_event.clear(); self._interrupt.clear(); self._cancel_event.clear(); self._commands=queue.Queue(); dlg.clear_audio_queue(); self._thread=threading.Thread(target=self._run,name="akira-voice",daemon=True); self._thread.start()
    def stop(self):
        self._stop=True; self._stop_event.set(); self._interrupt.set(); self._set_dialogue(False); self._commands.put(("stop",None)); thread=self._thread
    ...
    def _safe_transcribe(self,dialogue,audio):
        ...
        except Exception as exc: self.error.emit(str(exc) or "Не удалось распознать речь."); return ""
        return "" if not text or _is_hallucination(text) else str(text).strip()
    ...
    def _capture(self,dialogue=None):
        dialogue=dialogue or dlg; self._cancel_event.clear(); self._listening=False; self.mic_capture.emit(True); self._emit_state(self.LISTENING)
        try: audio=dialogue.record_utterance(timeout=dialogue.DIALOGUE_TIMEOUT,cancel_event=self._cancel_event)
        except Exception as exc: self.error.emit(str(exc) or "Не удалось записать речь."); audio=None
        finally: self.mic_capture.emit(False)
        if audio is None: self._listening=True; self._emit_state(self.IDLE); return
        text=self._safe_transcribe(dialogue,audio)
        if text: self._emit_state(self.THINKING); self.text_ready.emit(text); return
    ...
    def _wake_listen(self,dialogue=None):
        ...
        self._listening=True; emit=getattr(self,"_emit_state",None)
        if callable(emit): emit(self.LISTENING)
        try: audio=dialogue.record_utterance(timeout=getattr(dialogue,"WAKE_TIMEOUT",None),end_silence_ms=getattr(dialogue,"WAKE_END_SILENCE_MS",None),cancel_event=self._interrupt)
        except TypeError:
    ...
    def _run(self):
        ...
        stream=None
        try: stream=sd.InputStream(samplerate=dlg.SAMPLE_RATE,channels=1,dtype="float32",blocksize=dlg.FRAME_SAMPLES,callback=dlg.audio_callback); stream.__enter__(); self._audio_ok=True
        except Exception: self.error.emit("Не удалось открыть микрофон. Проверь разрешение macOS для приложения Akira.")
        try:
            while not self._stop_event.is_set():
                ...
                if not self._audio_ok or not self._listening: time.sleep(0.02); continue
