...
class ProactiveMainWindow(MainWindow):
    ...
    def _enter_voice_wake_dialogue(self):
        if not self.voice.is_dialogue(): self.voice.set_dialogue(True)
        self.voice.resume(); self._set_state(self.LISTENING)
        self.status.setText("Слушаю.")
        self.status.setStyleSheet("color: #c0c0c8; font-size: 12px; background: transparent;")
        self.input.setEnabled(True); self.input.setFocus()

    def _proactive_text(self, item):
        return str(item.get("message") or item.get("text") or item.get("title") or "").strip()
    ...
    def _submit_proactive_answer(self, message):
        if self.proactive_surface.active_question_id is None: return False
        self._append_message(message, "user")
        result = self.proactive_surface.answer(message)
        if result.get("success"):
            self._clear_status()
            if self.voice.is_dialogue(): self._set_state(self.LISTENING); self.voice.resume()
            else: self._set_state(self.IDLE)
        else: self._show_error("Не удалось передать ответ Акире.")
        return True
    ...
    def _on_submit(self, message):
        ...
        if self._submit_proactive_answer(message): return
        super()._on_submit(message)

    def _on_voice_text(self, text):
        if not text: return
        if self._is_wake_only(text): self._enter_voice_wake_dialogue(); return
        if self._submit_proactive_answer(text): self._last_voice = True; return
