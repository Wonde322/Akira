...
class UnifiedAgentLoop:
    """
    Host-owned agent execution loop.
    ...
    def _decide(self, goal, state):
        ...
        # Prefer explicit structured entry points.
        for method_name in (
            "decide",
            "next_action",
            "agent_step",
        ...
        return {
            "type": "finish",
            "status": "failed",
            "error": "No supported brain entry point",
        }
    ...
    def _authorize(self, action, arguments):
        ...
        if not callable(method):
            return {
                "authorized": True,
                "reason": "Safety adapter unavailable",
            }
    ...
    def _verify(
        ...
        if not callable(method):
            return {
                "status": "unknown",
                "verified": False,
                "reason": "Verifier has no verify method",
            }
    ...
    def run(self, goal):
        ...
        while state["iteration"] < self.max_iterations:
            ...
            decision = self._decide(goal, state)
            ...
            if not authorization.get("authorized", False):
                step.result = {
                    ...
                    "error": (
                        ...
                        or "Action not authorized"
                    ),
                }

                state["history"].append(
                    self._step_dict(step)
                )
            ...
            if verified and decision.get(
                "complete_after_action",
                False,
            ):
                return {
                    "success": True,
